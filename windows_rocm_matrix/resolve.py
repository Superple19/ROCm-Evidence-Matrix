...
def resolve_candidates(history, gfx, channel=None, rocm_version=None, torch_series=None, python_tag=None, include_unavailable=False):
    ...
    for candidate in history["candidates"]:
        ...
        if rocm_version and candidate["rocm_version"] != rocm_version:
            continue
        if torch_series and not candidate["torch_version"].startswith(torch_series + "."):
            continue
